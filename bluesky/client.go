...
var (
	...
	// the XrpcClient can still be actively used during).
	...
	// the XrpcClient blocks new API calls until the refresh finishes).
	jwtSyncRefreshThreshold = 2 * time.Minute
)

var (
	// ErrLoginUnauthorized is returned from a login attempt if the credentials
	// are rejected by the server or the local XrpcClient (master credentials).
...
// Client is an API XrpcClient attached to (and authenticated to) a Bluesky PDS instance.
type Client struct {
	XrpcClient *xrpc.Client // Underlying XRPC transport connected to the API
...
// XrpcClient and exchanges some basic information to ensure the connectivity works.
func DialWithClient(ctx context.Context, server string, client *http.Client) (*Client, error) {
	// Create the XRPC XrpcClient from the supplied HTTP one
	local := &xrpc.Client{
		Client: client,
		Host:   server,
	}
...
func (c *Client) Login(ctx context.Context, handle string, appkey string) error {
	...
	sess, err := atproto.ServerCreateSession(ctx, c.XrpcClient, &atproto.ServerCreateSession_Input{
		Identifier: handle,
		Password:   appkey,
	})
	if err != nil {
	...
	// Construct the authenticated XrpcClient and the JWT expiration metadata
...
// Close terminates the XrpcClient, shutting down all pending tasks and background
// operations.
...
func (c *Client) refresher() {
	for {
		...
		// Wait until some time passes or the XrpcClient is closing down
		select {
...
func (c *Client) maybeRefreshJWT() error {
	...
	// We've run out of the background refresh window, block the XrpcClient on a
	// synchronous refresh
	c.jwtLock.Lock()
...
// refreshJWT updates the JWT token and swaps out the credentials in the XrpcClient.
//
...
func (c *Client) refreshJWT(async bool) error {
	...
	// Attempt to refresh the JWT token. Since the XrpcClient might be used async
	// for other requests, create a copy with the fields we need to mess with.
	refClient := new(xrpc.Client)
	if async {
		c.jwtLock.RLock()
	}
	*refClient = *c.XrpcClient
	refClient.Auth = new(xrpc.AuthInfo)
	*refClient.Auth = *c.XrpcClient.Auth
	...
	// Update the JWT token in the local XrpcClient
	token, _, err := jwt.NewParser().ParseUnverified(sess.AccessJwt, jwt.MapClaims{})
	if err != nil {
		return err
	}
	...
	// Update the authenticated XrpcClient and the JWT expiration metadata
	if async {
		c.jwtLock.Lock()
		defer c.jwtLock.Unlock()
	}
...
// will receive an XRPC XrpcClient to do direct atproto calls through.
//
// Note, the caller should not hold onto the xrpc.Client. The XrpcClient is a copy
...
func (c *Client) CustomCall(callback func(client *xrpc.Client) error) error {
	...
	// Create a copy of the xrpc XrpcClient for power users
	dangling := new(xrpc.Client)

	c.jwtLock.RLock()
	*dangling = *c.XrpcClient
	*dangling.Auth = *c.XrpcClient.Auth

	if c.XrpcClient.AdminToken != nil {
		dangling.AdminToken = new(string)
		*dangling.AdminToken = *c.XrpcClient.AdminToken
	}
	if c.XrpcClient.UserAgent != nil {
		dangling.UserAgent = new(string)
		*dangling.UserAgent = *c.XrpcClient.UserAgent
	}
	c.jwtLock.RUnlock()

	// Run the user's callback against the copy of the authorized XrpcClient
