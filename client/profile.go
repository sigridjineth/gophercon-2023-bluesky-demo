...
// ResolveFollowing resolves the full list of followees of a profile and injects
// it into the profile itself.
//
// Note, since there is a fairly low limit on retrievable followees per API call,
// this method might take a while to complete on larger accounts. You may use the
// StreamFollowing to have finer control over the rate of retrievals,
// interruptions and memory usage.
func (p *Profile) ResolveFollowing(ctx context.Context) error {
	followeec, errc := p.StreamFollowing(ctx)
...
// StreamFollowing gradually resolves the full list of followees of
// a profile, feeding them async into a result channel, closing the channel when
...
func (p *Profile) StreamFollowing(ctx context.Context) (<-chan *User, <-chan error) {
	var (
		cursor    string
